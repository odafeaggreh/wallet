...
//Hordings / My holdings

export const getHoldimgsBegin = () => ({
  type: GET_HOLDINGS_BEGIN,
});

export const getHoldimgsSuccess = (myHoldings) => ({
  type: GET_HOLDINGS_SUCCESS,
  payload: { myHoldings },
});

export const getHoldimgsFailure = (error) => ({
    ...
    return axios({
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          dispatch(getHoldimgsSuccess(myHoldings));
        } else {
          dispatch(getHoldimgsFailure(response.data));
        }
      })
      .catch((error) => {
        dispatch(getHoldimgsFailure(error));
      });
  };
}
